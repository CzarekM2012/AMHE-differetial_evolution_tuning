...
def plot_series(n, series1, series2, series3, labels=("DE", "DE+MSR", "DE+PSR"),
                title="Title", xlabel="Generation",
                ylabel="Value", filename="results/plot.png"):
...
def experiment_track(func: callable, dimension: int = 10, generations: int = 2000):
    ...
    msr_model = DifferentialEvolution(evaluation_function=func, mutation_step_tuner=MSRTuner())
    psr_model = DifferentialEvolution(evaluation_function=func, mutation_step_tuner=PSRTuner())

    initial_population = np.random.uniform(low=-100., high=100., size=(pop_size, dimension))
    ...
    plot_series(generations, basic_stats["bests"], msr_stats["bests"], psr_stats["bests"],
                title="Najlepsze rozwiązanie w kolejnych generacjach",
                xlabel="Generacja", ylabel="Najlepsza wartość rozwiązania",
                filename=f"results/best-D{dimension}-{func.__name__}.png")
    plot_series(generations, basic_stats["means"], msr_stats["means"], psr_stats["means"],
                title="Średnia wartość rozwiązania w kolejnych generacjach",
                xlabel="Generacja", ylabel="Srednia wartość funkcji celu",
                filename=f"results/mean-D{dimension}-{func.__name__}.png")
    plot_series(generations, basic_stats["stds"], msr_stats["stds"], psr_stats["stds"],
                title="Odchylenie standardowe w kolejnych generacjach",
                xlabel="Generacja", ylabel="Odchylenie standardowe",
    ...
    for func in all_funcs:
        ...
        for _ in range(NUM_RUNS):
            initial_population = np.random.uniform(
                low=-100., high=100., size=(pop_size, dimension)
                )
            start = time.time()
            result, _ = basic_model.evolve(initial_population, number_of_generations=generations)
            end = time.time()
            basic_times.append(end-start)
            basic_bests.append(np.nanmin(func(result)))

            start = time.time()
            result, _ = msr_model.evolve(initial_population, number_of_generations=generations)
            end = time.time()
            msr_times.append(end-start)
            msr_bests.append(np.nanmin(func(result)))

            start = time.time()
            result, _ = psr_model.evolve(initial_population, number_of_generations=generations)
            end = time.time()
            psr_times.append(end-start)
            psr_bests.append(np.nanmin(func(result)))
        collect_stats(stats, basic_bests, msr_bests, psr_bests, basic_times, msr_times, psr_times)
    return pd.DataFrame(stats)


def collect_stats(stats_dict: dict, basic_bests, msr_bests, psr_bests, 
                  basic_times, msr_times, psr_times):
